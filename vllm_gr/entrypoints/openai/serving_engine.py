...
async def _add_batch_step(
    ...
    return output, internal_ids, engine_core_requests
...
async def beam_search(
    ...
    request_preprocess_time: float | None = None,
) -> AsyncGenerator[RequestOutput, None]:
    handler_time: float = 0.0
    if request_start_time is not None:
        handler_time = time.perf_counter() - request_start_time
    request_preprocess_time = request_preprocess_time or 0.0
    ...
    for token in range(max_tokens - pre_calc):
        ...
        if use_mega_request and fork_info is not None:
            ...
            output, new_ids, engine_core_requests = await _add_batch_step(
                self.engine_client,
                prompts_batch,
                lora_req_batch,
                request_id_batch,
            ...
            if token == 0:
                first_metrics = getattr(output[0], "metrics", None) if output else None
                if first_metrics is not None:
                    queued_ts = getattr(first_metrics, "queued_ts", 0.0)
                    scheduled_ts = getattr(first_metrics, "scheduled_ts", 0.0)
                    if queued_ts and scheduled_ts:
                        queue_time = scheduled_ts - queued_ts
                elif engine_core_requests:
                    # EngineCoreRequest metrics are available before output
                    # metrics on some patched engine paths.
                    first_stats = getattr(engine_core_requests[0], "metrics", None)
                    queued_ts = getattr(first_stats, "queued_ts", 0.0)
                    scheduled_ts = getattr(first_stats, "scheduled_ts", 0.0)
                    if queued_ts and scheduled_ts:
                        queue_time = scheduled_ts - queued_ts
        ...
        step_duration = time.perf_counter() - gen_start
        if token == 0:
            prefill_time += step_duration
        else:
            generation_time += step_duration
            decode_time += step_duration
        ...
        if token == 0:
            first_step_postprocess_time += postprocess_duration
        else:
            decode_postprocess_time += postprocess_duration
    ...
    metrics = RequestStateStats(
        ...
        first_step_postprocess_time=first_step_postprocess_time,
